...
{% for i in results_list %}
    ...
    From (
        ...
        objective,
        outbound_clicks.action_type as outbound_clicks_action_type,
        outbound_clicks.value as outbound_clicks_value,
        reach,
        social_spend,
        spend,
        unique_clicks,
        ...
        ROW_NUMBER() OVER (PARTITION BY ad_id,date_start,outbound_clicks.action_type order by {{daton_batch_runtime()}} desc) row_num
        from {{i}}
            {{unnesting("OUTBOUND_CLICKS")}}
            {% if is_incremental() %}
