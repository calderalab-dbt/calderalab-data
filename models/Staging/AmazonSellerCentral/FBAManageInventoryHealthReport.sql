{% if var('AMAZONSELLER') and var('FBAManageInventoryHealthReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('FBAManageInventoryHealthReport_tbl_ptrn','%fbamanageinventoryhealthreport'),
...
{% for i in relations %}
    ...
    {% if is_incremental() %}
        ...
        where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('FBAManageInventoryHealthReport_lookback',2592000000) }},0) from {{ this }})
    {% endif %}
