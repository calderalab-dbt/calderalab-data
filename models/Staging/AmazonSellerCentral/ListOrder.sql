{% if var('AMAZONSELLER') and var('ListOrder',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
{% for i in relations %}
    ...
    from (
        ...
        SellerOrderId,
        DATE_ADD({{ timezone_conversion("PurchaseDate") }}, INTERVAL 8 HOUR) as PurchaseDate,
        DATE_ADD({{ timezone_conversion("LastUpdateDate") }}, INTERVAL 8 HOUR) as LastUpdateDate,
        OrderStatus,
        FulfillmentChannel,
        SalesChannel,
        OrderChannel,
        ...
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('ListOrder_lookback',2592000000) }},0) from {{ this }})
        {% endif %} 
    )
