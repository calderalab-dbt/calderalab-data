{% if var('AMAZONSELLER') and var('FlatFileReturnsReportByReturnDate',True) %}
    {{ config( enabled = True ) }}
{% else %}
        ...
        from (
            ...
            from {{i}}  a 
                ...
                {% if is_incremental() %}
                    ...
                    where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('FlatFileReturnsReportByReturnDate_lookback',2592000000) }},0) from {{ this }})
                {% endif %}  
        ) 
