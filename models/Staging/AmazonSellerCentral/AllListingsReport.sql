{% if var('AMAZONSELLER') and var('AllListingsReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('AllListingsReport_tbl_ptrn','%alllistingsreport'),
...
{% for i in relations %}
    ...
    price,
    quantity,
    DATE_ADD({{ timezone_conversion("open_date") }}, INTERVAL 8 HOUR) as open_date,
    image_url,
    item_is_marketplace,
    product_id_type,
    zshop_shipping_fee,
    ...
    from {{i}}
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('AllListingsReport_lookback',2592000000) }},0) from {{ this }})
        {% endif %}       
