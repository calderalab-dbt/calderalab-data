{% if var('AMAZONSELLER') and var('SalesAndTrafficReportByChildASIN',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('SalesAndTrafficReportByChildASIN_tbl_ptrn','%salesandtrafficreportbychildasin%'),
    ...
    {% endif %}
        ...
        '{{store|replace("`","")}}' as store,
            ...
            {% if is_incremental() %}
                ...
                where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('SalesAndTrafficReportByChildASIN_lookback',2592000000) }},0) from {{ this }})
            {% endif %} 
