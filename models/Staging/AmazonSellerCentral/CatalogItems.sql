{% if var('AMAZONSELLER') and var('CatalogItems',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('CatalogItems_tbl_ptrn','%catalogitems'),
...
{% for i in relations %}
    ...
    from {{i}} a
        ...
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('CatalogItems_lookback',2592000000) }},0) from {{ this }})
        {% endif %}
