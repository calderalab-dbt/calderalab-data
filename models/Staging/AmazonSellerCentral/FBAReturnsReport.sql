{% if var('AMAZONSELLER') and var('FBAReturnsReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
    ...
    from (
        ...
        from {{i}}    
            {% if is_incremental() %}
                ...
                where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('FBAReturnsReport_lookback',2592000000) }},0) from {{ this }})
            {% endif %}
