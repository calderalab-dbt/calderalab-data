{% if var('AMAZONSELLER') and var('InventoryLedgerDetailedReport',False) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('InventoryLedgerDetailedReport_tbl_ptrn','%ledgerdetail%'),
...
{% for i in relations %}
    ...
    DATE_ADD({{ timezone_conversion("ReportRequestTime") }}, INTERVAL 8 HOUR) as ReportRequestTime,
        ...
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('InventoryLedgerDetailedReport_lookback',2592000000) }},0) from {{ this }})
        {% endif %}  
        )
