{% if var('AMAZONSELLER') and var('ListingOffersForASIN',False) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('ListingOffersForASIN_tbl_ptrn','%listingoffersforasin'),
    ...
    {% endif %}
        ...
        from {{i}} 
            ...
            {% if is_incremental() %}
                ...
                where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('ListingOffersForASIN_lookback',2592000000) }},0) from {{ this }})
            {% endif %} 
