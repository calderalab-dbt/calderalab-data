{% if var('AMAZONSELLER') and var('FBAManageInventory',False) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('FBAManageInventory_tbl_ptrn','%fbamanageinventory'),
...
{% for i in relations %}
    ...
    from {{i}}   
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('FBAManageInventory_lookback',2592000000) }},0) from {{ this }})
        {% endif %} 
