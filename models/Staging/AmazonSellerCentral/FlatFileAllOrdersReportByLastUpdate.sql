{% if var('AMAZONSELLER') and var('FlatFileAllOrdersReportByLastUpdate',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('FlatFileAllOrdersReportByLastUpdate_tbl_ptrn','%selling%FlatFileAllOrdersReportbyLastUpdate'),
...
{% for i in relations %}
    ...
    from (
        ...
        from (
            ...
            DATE_ADD({{ timezone_conversion("last_updated_date") }}, INTERVAL 8 HOUR) as last_updated_date,
            order_status, 
            fulfillment_channel, 
            sales_channel,
            order_channel,
            ...
            {% if is_incremental() %}
                ...
                where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('FlatFileAllOrdersReportByLastUpdate_lookback',2592000000) }},0) from {{ this }})
            {% endif %}  
