{% if var('AMAZONSPADS') and var('SPPortfolio',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('sp_portfolio_tbl_ptrn','%sp%portfolio'),
exclude=var('sp_portfolio_tbl_exclude_ptrn',''),
...
{% for i in relations %}
    ...
    {% if is_incremental() %}
        ...
        where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sp_portfolio_lookback',2592000000) }},0) from {{ this }})
    {% endif %}
