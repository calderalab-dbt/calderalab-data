{% if var('AMAZONSBADS') and var('SBProductMetadata',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('sb_productmetadata_tbl_ptrn','%sponsoredbrands%productmetadata'),
exclude=var('sb_productmetadata_tbl_exclude_ptrn',''),
...
{% for i in relations %}
    ...
    {{unnesting("priceToPay")}}
        ...
        {% if is_incremental() %}
            ...
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sb_productmetadata_lookback',2592000000) }},0) from {{ this }})
        {% endif %}    
