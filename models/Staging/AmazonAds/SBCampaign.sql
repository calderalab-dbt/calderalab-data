{% if var('AMAZONSBADS') and var('SBCampaign',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('sb_campaign_tbl_ptrn','%Amazon%SponsoredBrands_Campaign'),
...
{% for i in relations %}
    ...
    from {{i}}
        ...
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sb_campaign_lookback',2592000000) }},0) from {{ this }})
        {% endif %}
