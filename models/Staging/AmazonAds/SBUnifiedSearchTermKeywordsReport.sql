{% if var('AMAZONSBADS') and var('SBUnifiedSearchTermKeywordsReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
{% for i in relations %}
    ...
    from {{i}}
        ...
        where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sb_unifiedsearchtermkeywords_lookback',2592000000) }},0) from {{ this }})
        {% endif %} 
