{% if var('AMAZONSDADS') and var('SDPortfolio',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('sd_portfolio_tbl_ptrn','%Display%Portfolio'),
...
{% for i in relations %}
   ...
   -- RequestTime,
    ...
    {% if is_incremental() %}
        ...
        where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sd_portfolio_lookback',2592000000) }},0) from {{ this }})
    {% endif %}
