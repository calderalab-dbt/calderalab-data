...
table_pattern=var('sd_producttargeting_tbl_ptrn','%sponsoreddisplay%producttargetingreport'),
...
{% for i in relations %}
    ...
    where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sd_producttargeting_lookback',2592000000) }},0) from {{ this }})
    {% endif %} 
