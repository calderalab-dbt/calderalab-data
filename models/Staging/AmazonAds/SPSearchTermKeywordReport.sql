{% if var('AMAZONSPADS') and var('SPSearchTermKeywordReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('sp_searchtermkeyword_tbl_ptrn','%sponsoredproducts_searchtermkeywordreport'),
exclude=var('sp_searchtermkeyword_tbl_exclude_ptrn',''),
...
{% for i in relations %}
    ...
    from {{i}} a
        ...
        where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sp_searchtermkeyword_lookback',2592000000) }},0) from {{ this }})
        {% endif %}            
