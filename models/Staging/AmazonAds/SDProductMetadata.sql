...
table_pattern=var('sd_productmetadata_tbl_ptrn','%sponsoreddisplay%productmetadata'),
exclude=var('sd_productmetadata_tbl_exclude_ptrn',''),
...
{% for i in relations %}
    ...
    {{unnesting("priceToPay")}}
        ...
        {% if is_incremental() %}
            ...
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sd_productmetadata_lookback',2592000000) }},0) from {{ this }})
        {% endif %}    
