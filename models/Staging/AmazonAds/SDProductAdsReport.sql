{% if var('AMAZONSDADS') and var('SDProductAdsReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
{% for i in relations %}
    ...
    from {{i}} a
        ...
        {% if is_incremental() %}
            ...
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sd_productads_lookback',2592000000) }},0) from {{ this }})
        {% endif %}    
