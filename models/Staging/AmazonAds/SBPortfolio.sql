{% if var('AMAZONSBADS') and var('SBPortfolio',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
{% for i in relations %}
    ...
    from {{i}} 
        ...
        {% if is_incremental() %}
            ...
            where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sb_portfolio_lookback',2592000000) }},0) from {{ this }})
        {% endif %}
