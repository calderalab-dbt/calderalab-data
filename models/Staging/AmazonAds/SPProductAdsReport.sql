{% if var('AMAZONSPADS') and var('SPProductAdsReport',True) %}
    {{ config( enabled = True ) }}
{% else %}
...
{% for i in relations %}
    ...
    from {{i}} a
        ...
        where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sp_productads_lookback',2592000000) }},0) from {{ this }})
        {% endif %}    
