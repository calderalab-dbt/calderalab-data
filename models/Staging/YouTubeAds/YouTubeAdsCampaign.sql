...
{{set_table_name('%caldera_google_ads_yt_campaign_performance%')}}    --
{% endset %}  
...
{% for i in results_list %}
    ...
    From (
        ...
        '{{env_var("DBT_CLOUD_RUN_ID", "manual")}}' as _run_id,
            ROW_NUMBER() OVER (PARTITION BY SEGMENTS.date,CAMPAIGN.id order by {{daton_batch_runtime()}} desc) row_num
	    from {{i}} 
            {{unnesting("campaign")}}
            {{unnesting("metrics")}}
            {{unnesting("segments")}}
