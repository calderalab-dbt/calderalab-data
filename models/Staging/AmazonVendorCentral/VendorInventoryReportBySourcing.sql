{% if var("AMAZONVENDOR") and var("VendorInventoryReportBySourcing",True) %} 
    {{ config(enabled=True) }}
{% else %} 
...
table_pattern=var('VendorInventoryReportBySourcing_tbl_ptrn','%vendorinventoryreportbysourcing%'),
exclude=var('VendorInventoryReportBySourcing_exclude_tbl_ptrn',''),
...
{% for i in relations %}
    ...
    (
        ...
        {% endif %}       
                        ...
                        WHERE c._daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('VendorInventoryReportBySourcing_lookback',172800000) }},0) from {{ this }})
                        {% endif %}
