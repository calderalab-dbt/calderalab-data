{% if var("AMAZONVENDOR") and var("VendorSalesReportBySourcing",True) %} 
    {{ config(enabled=True) }}
{% else %} 
...
table_pattern=var('VendorSalesReportBySourcing_tbl_ptrn','%vendorsalesreportbysourcing%'),
exclude=var('VendorSalesReportBySourcing_exclude_tbl_ptrn',''),
...
{% for i in relations %}
    ...
    (
        ...
        {% if is_incremental() %}
            ...
            WHERE c._daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('VendorSalesReportBySourcing_lookback',172800000) }},0) from {{ this }})
        {% endif %}
